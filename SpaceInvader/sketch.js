...
function loseScreen() {
	...
	textSize(30)
	fill(222)
	text(`Balas ...... ${p.singleLeft} =  $ ${bulletScore}`, 50, HEIGHT / 2)
	text(`Bombas ...... ${p.bombLeft} =  $ ${bombScore}`, 50, HEIGHT / 2 + 40)
	text(`Escudos ...... ${p.shieldLeft} =  $ ${shieldScore}`, 50, HEIGHT / 2 + 80)
	text(`Punteria ...... ${floor(enemiesKilled / (p.shotCount + 1) * 100)} % =  $ ${accuracyScore}`, 50, HEIGHT / 2 + 120)

	text(`Nave ...... ${p.lives} =  $ ${shipScore}`, WIDTH / 2, HEIGHT / 2)
	text(`Soldados ...... ${soldierSafeCount} =  $ ${soldierScore}`, WIDTH / 2, HEIGHT / 2 + 40)
	text(`Perdiste ...... ${loseScore} =  $ ${loseScore}`, WIDTH / 2, HEIGHT / 2 + 80)
	text(`Enemigos ...... ${enemiesKilled} % =  $ ${enemyScore}`, WIDTH / 2, HEIGHT / 2 + 120)

	textAlign(CENTER, CENTER)
	fill(255, 234 , 0)
	textSize(40)
	text(`TOTAL = $ ${totalScore}`, WIDTH / 2, HEIGHT / 2 + 240)
}
...
function winScreen() {
	...
	const bombScore = p.bombLeft * 25;
	const shieldScore = p.shieldLeft * 50;
	const accuracyScore = (bulletScore + bombScore + shieldScore) * (parseInt(enemiesKilled / (p.shotCount + 1)));
	const shipScore = p.lives * 200;
	const soldierScore = soldierSafeCount * 250;
	const winScore = 2000;
	const enemyScore = enemiesKilled * 100
	...
	text(`Nave ...... ${p.lives} =  $ ${shipScore}`, WIDTH / 2, HEIGHT / 2)
	text(`Soldados ...... ${soldierSafeCount} =  $ ${soldierScore}`, WIDTH / 2, HEIGHT / 2 + 40)
	text(`Ganaste ...... ${winScore} =  $ ${winScore}`, WIDTH / 2, HEIGHT / 2 + 80)
	text(`Enemigos ...... ${enemiesKilled} % =  $ ${enemyScore}`, WIDTH / 2, HEIGHT / 2 + 120)
...
function setup() {
	...
	// creation of armyMan Array - 4 soldiers
	for (let i = 0; i < 6; i++){
